...
public interface ApprovalLineMapper {
    ...
    // 결재선 등록 조직도 가져오기
    @Select("SELECT PI.EMP_NO ,PI.NAME ,PI.POSITION_CODE , P.NAME_OF_POSITION , PI.DEPT_CODE , D.PARTNAME ,PI.TEAM_CODE " +
            ", DT.TEAM_NAME FROM PERSONNEL_INFORMATION PI JOIN POSITION P ON PI.POSITION_CODE = P.POSITION_CODE " +
            "JOIN DEPARTMENT D ON PI.DEPT_CODE = D.DEPT_CODE JOIN DEPARTMENT_TEAM DT ON PI.TEAM_CODE = DT.TEAM_CODE " +
            "WHERE LEAVING_DATE IS NULL ORDER BY P.POSITION_CODE, D.DEPT_CODE, DT.TEAM_CODE, PI.NAME")
    ...
    @Select("SELECT AT.APPR_LINE_NO, AT.APPR_LINE_NAME , AT.CREATED_DATE, DT.TEMPLATE_NO , DT.TITLE , DT.CONTENT " +
            ...
            "JOIN DOC_TEMPLATE_CATEGORY DTC ON DT.CATEGORY_NO = DTC.CATEGORY_NO WHERE WRITER_NO IS NULL ORDER BY CREATED_DATE DESC")
